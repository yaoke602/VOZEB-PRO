...
- [界面] 爆款复刻对齐公司原型的紧凑视频入口、主体原/新图对照和三栏分镜工作区，补充参考素材引用状态、当前版本标记及移动端主体编辑状态；继续复用原有生成、计费与项目保存逻辑。

- 爆款复刻分镜按当前替换主体改写描述，避免旧外观、商品名称和历史设定覆盖新参考图，并完整保留已选素材；规划完成后自动回填本次公开分镜描述。

- 爆款复刻分镜视频精简上游提示词，保留 Agent 规划与真实参考绑定，避免重复附加内部规划和资产元数据。

- [创作] 新增爆款复刻四步工作区：原视频、主体设定、分镜与视频成片；支持本地上传、资源库引用、真实采样帧分析、主体图片生成、分镜视频生成和成片合成，复用现有项目存储、Agent、计费与媒体链路。

- [渠道] 新增快客云视频协议预设，支持多媒体参考、嵌套任务 ID 和异步查询；提交按所选清晰度发送 resolution（已保存渠道同样生效）。查询异常保留原任务，不误判生成失败。真实平台完整验收待完成。

- [千川] 新增 `/analytics` 原生数据分析入口、OAuth 授权与手动同步；补充基于已同步数据的 AI 查数、服务端回答恢复和千川账户/查询/同步 MCP 工具。沿用现有模型、计费、PostgreSQL 与用户权限，不增加 DataEase，不自动修改投放；真实平台与生产部署仍待验收。补齐结构化文本无效成功响应的退款回调。
