...
如果远端提交与当前提交相同，并且实际 App 容器镜像、`.env` 和镜像 revision 标签都指向该提交，默认不重复构建，只执行 Compose 状态与应用健康检查。没有 App 但保留持久卷的现场不能进入该快速路径，必须先完成备份再按完整发布流程验证。如果源码已经更新、但上次执行在版本确认或构建阶段中止，脚本会继续完成尚未发布的目标镜像。维护者也可以使用 `--force-build` 强制重建。
...
脚本在修改 `.env` 前记录旧应用镜像。进入发布切换后会为正常退出、`INT` 和 `TERM` 设置事务式恢复处理；如果 App 在新镜像下未通过健康检查或脚本意外中断，脚本通过权限受控的临时文件校验并原子恢复旧 `.env`，使用旧镜像重建 App 和 Worker，再次检查健康状态。第一次部署没有旧 App 时，失败恢复只移除本次创建的 App/Worker 容器，保留 PostgreSQL 与命名卷。
